...
export default function RouletteScreen({route}) {
  ...
  const SkorApi = async (email,skor) => {
    axios.get(`10.0.2.2/api/addSkor`, {email: email,skor: skor,});

  };

  const [chipAmount, setChipAmount] = useState(100000);
  ...
  const rotateWheelForAmount = (amount) => {
    rotationAmount.value = withTiming(
      rotationAmount.value + 1440 + amount * 24,
      {
  ...
  const calculateWinnings = (color) => {
    setTimeout(() => {
      setBetAmount(0);
      let place =
        ((Math.floor(rotationAmount.value + 0.99) % 360) / 24 + 10) % 15;
      if (color === "GREEN" && place === 0) {
        setChipAmount((prevState) => prevState + betAmount * 13);
        console.log("kazandın"+betAmount * 13);
        const response =  axios.get(`http://10.0.2.2:8000/api/addSkor`,
        {
          params: {
            email: email,
            skor:betAmount*13,
        ...
        if (color === "BLACK" && place % 2 === 0) {
          ...
          console.log("kazandın"+betAmount);
          const response =  axios.get(`http://10.0.2.2:8000/api/addSkor`,
          {
            params: {
              email: email,
          ...
          if (color === "RED" && place % 2 !== 0) {
            ...
            setChipAmount((prevState) => prevState - betAmount);
            console.log("kaybettin"+betAmount);
    ...
    <View style={styles.container}>
      ...
      <Animated.View style={animatedRotationStyle}>
        <Image
          ...
          source={require("../components/Images/roulete.png")}
        />
      </Animated.View>
      ...
      <View style={{ gap: 15, alignItems: "center" }}>
        ...
        <View style={{ alignItems: "center" }}>
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            <TouchableOpacity
              onPress={lowerBetAmount}
              style={styles.changeBetAmountButton}
            >
              <Text>{"-"}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={increaseBetAmount}
              style={styles.changeBetAmountButton}
            >
              <Text>{"+"}</Text>
            </TouchableOpacity>
        ...
        <View style={{ flexDirection: "row", gap: 5 }}>
          ...
          <TouchableOpacity
            onPress={spinBettingOnBlack}
            disabled={wheelIsSpinning}
            style={[styles.betButton, { backgroundColor: "black" }]}
          >
            <Text style={{ color: "white" }}>{"2X"}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={spinBettingOnGreen}
            disabled={wheelIsSpinning}
            style={[styles.betButton, { backgroundColor: "green" }]}
          >
